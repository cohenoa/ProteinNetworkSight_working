...
def execute_batch(session, batch):
    ...
    # session.run("""
    #     UNWIND $batch AS row
    #     MATCH (p1:Protein {string_protein_id: row.protein1})
    #     MATCH (p2:Protein {string_protein_id: row.protein2})
    #     MERGE (p1)-[:PHYSICAL_LINK {score: toInteger(row.score)}]->(p2);
    # """, batch=batch)

def delete_batch(session, batch):
    session.run("""
        UNWIND $batch AS row
        MATCH (p1:Protein {string_protein_id: row.protein1})
        MATCH (p2:Protein {string_protein_id: row.protein2})
...
def create_protein_links_batch(session, file_path, batch_size=2, max_workers=5, max_queue_size=10):
        ...
        with tqdm(total=total_rows, desc="Processing Links", unit=" rows") as pbar, ThreadPoolExecutor(max_workers=max_workers) as executor:
            for row in reader:
                ...
                if len(batch) >= batch_size:
                    ...
                    future = executor.submit(execute_batch, session, batch_copy)
...
# def create_protein_links_batch(session, file_path, batch_size=10):
#     with open(file_path, "r") as file:
#         reader = csv.DictReader(file)
#         total_rows = sum(1 for _ in open(file_path)) - 1  # Get total rows (minus header)
...
# def create_protein_links_batch(session, file_path, batch_size=10000):
#     with open(file_path, "r") as file:
#         reader = csv.DictReader(file)
#         total_rows = sum(1 for _ in open(file_path)) - 1  # Get total rows (minus header)
...
if __name__ == "__main__":
    ...
    create_protein_links_batch(session, PATH)
